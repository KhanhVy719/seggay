(() => {
    ...
    async function parseCarrierBuffer(arrayBuffer, expected) {
        ...
        // Find MPEG-TS sync byte 0x47
        // TS packets are 188 bytes long. We verify 0x47 is followed by another 0x47 188 bytes later to be sure.
        let tsOffset = -1;
        // In some rare cases, the first sync byte might be near the end, but usually it's within the first few KB
        ...
        if (tsOffset === -1) {
            throw new Error('Could not find MPEG-TS sync byte 0x47 in PNG carrier file');
        }
